...
describe("Wordle Server", () => {
  describe("GET /newgame starts a new game and returns the session ID", () => {
    ...
    describe("returns a session ID", () => {
      test("body.sessionID should exist", () =>
        expect(res.body.sessionID).toBeDefined());
      test("the session ID is a valid uuid", () =>
        expect(uuid.validate(res.body.sessionID)).toBe(true));
    });
  });
  ...
  describe("GET /gamestate returns correct game state after a new game is made", () => {
    ...
    beforeAll(async () => {
      ...
      gameStateRes = await agent.get(`/gamestate?sessionID=${sessionID}`);
      gameState = gameStateRes.body.gameState;
    });

    test("returns a status code 200", () =>
      expect(gameStateRes.status).toBe(200));
    describe("game state object is accurate", () => {
      test("guesses is an empty array", () =>
        expect(gameState.guesses.length).toBe(0));
      test("remainingGuesses is 6", () =>
        expect(gameState.remainingGuesses).toBe(6));
      test("wrongLetters is an empty array", () =>
        expect(gameState.wrongLetters).toEqual([]));
      test("closeLetters is an empty array", () =>
        expect(gameState.closeLetters).toEqual([]));
      test("rightLetters is an empty array", () =>
        expect(gameState.rightLetters).toEqual([]));
    });
  });
  ...
  describe("GET /newgame with answer query sets the wordToGuess and returns the session ID", () => {
    ...
    test("guessing the specified answer should win", () =>
      expect(res2.body.gameState.gameOver).toBe(true));
  });
  ...
  describe("POST /guess returns updated game state with correctness indicators", () => {
    ...
    beforeAll(async () => {
      ...
      let sessionID = res.body.sessionID;
      // Make a guess
      guessRes = await agent.post("/guess").send({ guess: "phase", sessionID });
    ...
    describe("the game state object is accurate", () => {
      test("there should be 1 guess in guesses", () =>
        expect(gameState.guesses.length).toBe(1));
      test("the guess is an array with the correct values", () =>
        expect(gameState.guesses[0]).toEqual([
      ...
      test("remaining guesses is 5", () =>
        expect(gameState.remainingGuesses).toBe(5));
      test("wrongLetters contains only the wrong guessed letters", () =>
        expect(gameState.wrongLetters.sort((a, b) => (a > b ? 1 : -1))).toEqual(
          ["h", "s"]
        ));
      test("closeLetters contains only the close guessed letters", () =>
        expect(gameState.closeLetters.sort((a, b) => (a > b ? 1 : -1))).toEqual(
          ["a", "p"]
        ));
      test("rightLetters contains only the correctly guessed letters", () =>
        expect(gameState.rightLetters).toEqual(["e"]));
    });
  });
  ...
  describe("POST /guess returns 400 if no session ID is provided", () => {
    let res;
    beforeAll(async () => {
      res = await request(server).post("/guess").send({ guess: "apple" });
    });
  ...
  describe("GET /gamestate returns correct game state after two guesses", () => {
    ...
    beforeAll(async () => {
      ...
      await agent.post("/guess").send({ guess: "angle", sessionID });
      gameStateRes = await agent.get(`/gamestate?sessionID=${sessionID}`);
      gameState = gameStateRes.body.gameState;
    });
    ...
    test("returns a status code 200", () =>
      expect(gameStateRes.status).toBe(200));
    describe("game state object is accurate", () => {
      test("guesses has 2 elements", () =>
        expect(gameState.guesses.length).toBe(2));
      test("guesses[0] represents the first guess", () =>
        expect(gameState.guesses[0]).toEqual([
          { value: "p", result: "CLOSE" },
          { value: "h", result: "WRONG" },
          { value: "a", result: "CLOSE" },
      ...
      test("guesses[1] represents the 2nd guess", () =>
        expect(gameState.guesses[1]).toEqual([
      ...
      test("closeLetters contains only the close guessed letters", () =>
        expect(gameState.closeLetters).toEqual(["p"]));
      test("rightLetters contains only the right guessed letters", () =>
        expect(gameState.rightLetters.sort((a, b) => (a > b ? 1 : -1))).toEqual(
          ["a", "e", "l"]
        ));
    });
  });
  ...
  describe("GET /gamestate returns 400 if no session ID is provided", () => {
    let res;
    beforeAll(async () => {
      res = await request(server).get("/gamestate");
    });
  ...
  describe("GET /gamestate returns 404 if session ID does not match any active session", () => {
    ...
    beforeAll(async () => {
      const badID = uuid.v4(); // Simulate non-existent session ID
      res = await request(server).get(`/gamestate?sessionID=${badID}`);
    });
  ...
  describe("POST /guess returns 400 for invalid guess (not 5 letters)", () => {
    ...
    describe("doesn't accept short guesses", () => {
      let res;
      beforeAll(async () => {
        res = await agent.post("/guess").send({ guess: "app", sessionID }); // Invalid guess, less than 5 letters
      });
      test("status code should be 400", () => expect(res.status).toBe(400));
      test("response should include an error message", () =>
        expect(res.body.error).toBeDefined());
    });
    describe("doesn't accept long guesses", () => {
      let res;
      beforeAll(async () => {
        res = await agent.post("/guess").send({ guess: "orange", sessionID }); // Invalid guess, less than 5 letters
      });
      test("status code should be 400", () => expect(res.status).toBe(400));
      test("response should include an error message", () =>
        expect(res.body.error).toBeDefined());
  ...
  describe("POST /guess returns 400 for invalid guess (non-letter characters)", () => {
    ...
    describe("doesn't accept numbers", () => {
      let res;
      beforeAll(async () => {
        res = await agent.post("/guess").send({ guess: "app1e", sessionID }); // Invalid guess, less than 5 letters
      });
      test("status code should be 400", () => expect(res.status).toBe(400));
      test("response should include an error message", () =>
        expect(res.body.error).toBeDefined());
    ...
    describe("doesn't accept special characters", () => {
      let res;
      beforeAll(async () => {
        res = await agent.post("/guess").send({ guess: "@pple", sessionID }); // Invalid guess, less than 5 letters
      });
      test("status code should be 400", () => expect(res.status).toBe(400));
      test("response should include an error message", () =>
        expect(res.body.error).toBeDefined());
  ...
  describe("DELETE /reset starts a new game with the same ID", () => {
    ...
    describe("responds with a new game state object", () => {
      test("wordToGuess is undefined", () =>
        expect(gameState.wordToGuess).toBeUndefined());
      test("guesses is an empty array", () =>
    ...
    test("the original ID still references the session", () =>
      expect(checkRes.body.gameState).toBeDefined());
  });
  ...
  describe("DELETE /reset returns 404 if session ID does not match any active session", () => {
    ...
    beforeAll(async () => {
      ...
      await agent
        .post("/guess")
        .send({ guess: "phase", sessionID: res.body.sessionID });
  ...
  describe("DELETE /delete removes session from the active sessions", () => {
    ...
    test("the old ID should no longer reference an active session", () =>
